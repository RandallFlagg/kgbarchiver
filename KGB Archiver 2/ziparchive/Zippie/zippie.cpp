...
void ProcessData(CZipArchive& zip, CCmdLine& cmd, CZipIndexesArray& vRevised, bool bExtract)
{
		...
		{
			ZIP_U16_U64 iMax = zip.GetCount();
			for (ZIP_U16_U64 i = 0; i < iMax; i++)
				vRevised.Add(i);
		}
		else
		{
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		{
			FILELIST lFiles;
			ZIP_U16_U64 iCount = zip.GetCount();
