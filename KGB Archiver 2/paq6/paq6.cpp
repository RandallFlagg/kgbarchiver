// paq6.cpp : Defines the entry point for the DLL application.
//
...
void kgbdecompress(FILE *archive, char *data, unsigned __int64 size){
	if(kgbEnc == NULL)
		kgbEnc = new Transformer(DECOMPRESS, archive);

	/*for(unsigned __int64 i=0;i<size;i++){
		data[i] = kgbEnc->decode();
	}*/
	char *last = &data[size];
	for(;data!=last;data++){
		*data = kgbEnc->decode();
					 ...
					 )
{
	switch(ul_reason_for_call){
		case DLL_PROCESS_DETACH: if(kgbEnc != NULL) kgbEnc->flush(); break;
	}
