...
/*void sortFiles(vector<string> &files){
	string common[6][35] = {{"txt", "htm", "html", "xml", "css", "php", "php3", "php4", "php5", "asp", "vcproj", "sln", "dev", "cpp", "c", "h", "pl" /*perl* /, "rc", "pas", "iss" /*inno setup* /, "vb", "js", "vbs", "bat", "rtf", "reg", "key", "csv", "cue", "ini", "inf", "doc", "xls", "suo", ""},
...
/*DWORD WINAPI compress(LPVOID lpParam){	
   ...
   char filebuff[32767]; //maxymalny rozmiar bufora zapisu
   char readbuff[65536];
   __int64 rbuffSize=0;
...
//MessageBox(0, "Odczytywanie rozmiarów plików", "", 0);
   // Get file sizes
...
//MessageBox(0, "sprawdzamy czy na liœcie cos jes", "", 0);
...
//MessageBox(0, "otwieramy archiwum", "", 0);    
    // Write header
...
//MessageBox(0, "w³¹czamy buforowanie", "", 0);
	if(setvbuf(archive, filebuff, _IOFBF, sizeof(filebuff)) != NULL){
		return false;
	}
//MessageBox(0, "dodajemy sfx", "", 0);
	if(sfx_arch){ //do³¹czamy modu³ sfx
		...
		char buffer[SFX_OFFSET];
		for(int i=0;i<SFX_OFFSET-1;i++)
			buffer[i] = '\0';
...
//MessageBox(0, "zapisujemy nag³ówek", "", 0);
...
//MessageBox(0, "tworzymy obiekty kompresora", "", 0);    
	// Write data
    Transformer e(COMPRESS, archive);
    __int64 file_start=ftell(archive);
    for (int i=0; i<int(filename.size()); ++i) {
      ...
      if (size>=0) {
        ...
        _done_all += _size;
        _compressed = file_start = ftell(archive);
...
DWORD WINAPI decompress(LPVOID lParam/*char *archive_name*//*, vector<string> filenames*/){
    ...
    if(archive){
		...
		_compressed /*= compressed_bytes*/=ftell(archive);
		fseek(archive, 0L, SEEK_SET);
		
		if(tolower(archive_name[strlen(archive_name)-1]) == 'e' && tolower(archive_name[strlen(archive_name)-2]) == 'x' && tolower(archive_name[strlen(archive_name)-3]) == 'e' && tolower(archive_name[strlen(archive_name)-4]) == '.'){
			char buff[2048];
			for(int i=0;!feof(archive);i++){
				fread(buff, sizeof(*buff), sizeof(buff)/sizeof(*buff), archive);
				if(cmp(PROGNAME, buff)){
					fseek(archive, i*2048, SEEK_SET);
					break;
				}else if(cmp("ENC", buff)){
					//tu nic nie robimy, to sie zrobi gdzies wczesniej
				}
			}
			//fseek(archive, 552*1024, SEEK_SET);
...
/*bool getFilesFromArchive(char *arch, vector<string> &files){
	...
	if(tolower(arch[strlen(arch)-1]) == 'e' && tolower(arch[strlen(arch)-2]) == 'x' && tolower(arch[strlen(arch)-3]) == 'e'){
		fseek(archive, SFX_OFFSET, SEEK_SET);
		//_compressed -= SFX_OFFSET;
		sfx_arch = true;
	}
...
bool getFilesFromArchive(char *arch, vector<string> &files, vector<unsigned long> &filesizes){
	FILE* archive=fopen(arch, "rb");
