...
int encfile(FILE *fin, FILE *fout, aes_ctx ctx[1], const char* ifn, const char* ofn)
{   char            dbuf[3 * BLOCK_LEN];
    ...
    {   // if the file length is less than one block
        ...
        aes_enc_blk((unsigned char*)(dbuf + len), (unsigned char*)(dbuf + len), ctx);
    ...
    {   char    *b1 = dbuf, *b2 = b1 + BLOCK_LEN, *b3 = b2 + BLOCK_LEN, *bt;
        ...
        for( ; ; )
        {
            ...
            aes_enc_blk((unsigned char*)b1, (unsigned char*)b1, ctx);
            ...
            if(len != 0 && len != BLOCK_LEN)    // use ciphertext stealing
            {
                ...
                // encrypt this block
                aes_enc_blk((unsigned char*)b3, (unsigned char*)b3, ctx);
...
int decfile(FILE *fin, FILE *fout, aes_ctx ctx[1], const char* ifn, const char* ofn)
{   char            dbuf[3 * BLOCK_LEN], buf[BLOCK_LEN];
    ...
    if(len < 2 * BLOCK_LEN) // the original file is less than one block in length
    {
        ...
        aes_dec_blk((unsigned char*)(dbuf + len), (unsigned char*)(dbuf + len), ctx);
    ...
    {   char    *b1 = dbuf, *b2 = b1 + BLOCK_LEN, *b3 = b2 + BLOCK_LEN, *bt;
        ...
        for( ; ; )  // while some ciphertext remains, prepare to decrypt block b2
        {
            ...
            aes_dec_blk((unsigned char*)b2, (unsigned char*)buf, ctx);
            ...
            else    // partial last block - use ciphertext stealing
            {
                ...
                aes_dec_blk((unsigned char*)b3, (unsigned char*)b3, ctx);
